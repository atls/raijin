...
export class TypeScript extends EventEmitter {
  ...
  private async run(
    ...
    const beforeTransformer: typescript.TransformerFactory<
    ...
    const afterTransformer: typescript.TransformerFactory<
      typescript.SourceFile
    > = (_) => (sourceFile) => {
  ...
  private filterDiagnostics(
    ...
    return diagnostics
      ...
      .filter(
        (diagnostic) =>
          !(
            diagnostic.code === 1479 &&
            diagnostic.file?.fileName.includes("/.yarn/")
          )
      )
      .filter(
        (diagnostic) =>
      ...
      .filter(
        (diagnostic) =>
          !(
            ...
            diagnostic.file?.fileName.includes("/lexical/")
          )
      )
      .filter(
        (diagnostic) =>
          !(
            ...
            diagnostic.file?.fileName.includes("/@yarnpkg/libui/")
          )
      )
      .filter(
        (diagnostic) =>
