    ...
    describe('migrations', () => {
        ...
        it('should remove run from version workflow', async () => {
            const result = await schematicRunner
                ...
                .toPromise();
            expect(result.read('.github/workflows/version.yaml').toString()).toContain('yarn workspaces changed foreach --no-private --verbose version patch --deferred');
        });
        it('should remove run from publish workflow', async () => {
            const result = await schematicRunner
                .runSchematicAsync('library-workflows-remove-run', {}, tree)
                .toPromise();
            expect(result.read('.github/workflows/publish.yaml').toString()).toContain('yarn workspaces changed foreach --verbose --topological --no-private npm publish --access public');
        });
    });
});
