...
После подключения нового или существующего проекта выполните [настройку проверок перед коммитом](./docs/raijin/quickstart.ru.md#staged-checks). Raijin v2 требует явную конфигурацию lint-staged, принадлежащую проекту. Установленный hook вызывает `yarn commit staged`; без конфигурации коммит с подготовленными файлами завершится ошибкой.
...
При переходе на v2 выполните [тот же шаг настройки](./docs/raijin/quickstart.ru.md#staged-checks) до следующего коммита. Неявная конфигурация Raijin больше не используется; существующие native-конфиги lint-staged сохраняйте и проверяйте.
