...
- для обычных массивов использовать `Array<T>`
- исключения: `readonly T[]` и tuple (`[A, B]`) оставлять в квадратной записи
- `enum` не использовать; вместо него использовать запись через объект
- пример:
...
- `as const` разрешён (это не "произвольный каст", а фиксация литералов)
