...
class BadgesCommand extends BaseCommand {
    async execute() {
        ...
        const { project, workspace: projectWorkspace } = await Project.find(configuration, this.context.cwd);
        if (!projectWorkspace)
        ...
        const commandReport = await StreamReport.start({
            ...
            await report.startTimerPromise('Generating badges', async () => {
                ...
                const sortedLookup = miscUtils.sortMap([...lookupSet], (pkg) => structUtils.stringifyLocator(pkg));
                ...
                const atlsVersions = await Promise.all(BADGES.map(async (badge) => ({
                    name: badge,
                ...
                const versionsReducer = (badges, pkg) => {
                    const getColors = () => {
                        ...
                        const pair = Object.entries(COLORS).find(([pattern]) => pkg.name.search(pattern) !== -1);
                        if (pair) {
                            const [, colors] = pair;
                    ...
                    if (pkg.version) {
                        ...
                        const wrapWithLink = (content, link) => `[${content}](${link})`;
...
BadgesCommand.paths = [['badges', 'generate']];
BadgesCommand.VERSIONS_SEPARATOR = '[//]: # (VERSIONS)';
BadgesCommand.BADGE_URL = 'https://img.shields.io/static/v1';
BadgesCommand.BADGE_STYLE = 'for-the-badge';
BadgesCommand.REGISTRY_URL = 'https://npmjs.com';
BadgesCommand.REGISTRY_PACKAGE_PATH = '/package';
