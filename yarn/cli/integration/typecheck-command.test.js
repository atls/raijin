...
describe('yarn', () => {
    describe('commands', () => {
        describe('typecheck', () => {
            test('it should typecheck withouth errors', makeTemporaryEnv({
                dependencies: {
                    typescript: '^4.5.3',
                },
...
`);
        ...
        test('it should typecheck with errors', makeTemporaryEnv({
            dependencies: {
                typescript: '^4.5.3',
            },
        }, async ({ path, run, source }) => {
            await run('install');
            await xfs.writeFilePromise(`${path}/invalid.ts`, `
const s = (n: string) => n
s(5)
`);
            try {
                ...
                expect(error.stdout).toContain("Argument of type 'number' is not assignable to parameter of type");
