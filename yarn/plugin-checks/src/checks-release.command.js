...
class ChecksReleaseCommand extends BaseCommand {
    async execute() {
        const { project } = await Project.find(await Configuration.find(this.context.cwd, this.context.plugins), this.context.cwd);
        ...
        try {
            ...
            for await (const workspace of workspaces) {
                if (workspace.manifest.scripts.get('build')) {
                    ...
                    const code = await this.cli.run(['workspace', workspace.manifest.raw.name, 'build'], context);
                    if (code > 0) {
                        annotations.push({
...
ChecksReleaseCommand.paths = [['checks', 'release']];
