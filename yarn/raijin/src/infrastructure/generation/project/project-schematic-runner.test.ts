...
import { buildProjectSchematicArtifact } from './schematics/project-schematic-artifact.builder.js'

const expectedCommonArtifacts = [
...
const getArtifactPaths = (result: Awaited<ReturnType<typeof runProjectSchematic>>): Array<string> =>
...
const assertExactScaffoldArtifacts = async (scaffoldType: 'library' | 'project'): Promise<void> => {
  const target = await createProjectTarget(`${scaffoldType}-fixture`)
  const result = await runProjectSchematic(collectionPath, { scaffoldType, target })
  ...
  assert.deepEqual(
    getArtifactPaths(result),
    [...expectedCommonArtifacts, ...scaffoldArtifacts].sort()
  )
  assert.equal(
...
test('should generate exact project scaffold artifacts in a nested target', async () => {
  await assertExactScaffoldArtifacts('project')
})

test('should generate exact library scaffold artifacts in a nested target', async () => {
  await assertExactScaffoldArtifacts('library')
})

test('should preserve the existing workspace while applying the owned legacy migration', async () => {
  const scripts = {
  ...
  const result = await runProjectSchematic(collectionPath, {
    scaffoldType: 'project',
    target,
  })
...
test('should roll back partial tree writes when a schematic rule fails', async () => {
  ...
  assert.equal(result.failure.code, 'project-schematic-failed')
...
test('should return a typed failure for an unavailable collection provider', async () => {
  ...
  const result = await runProjectSchematic(join(fixtureRoot, 'missing/collection.json'), {
    scaffoldType: 'project',
    target,
  })
...
test('should return a typed failure for a corrupt collection artifact', async () => {
  ...
  const result = await runProjectSchematic(corruptCollectionPath, {
    scaffoldType: 'project',
    target,
  })
