...
import type { LegacyStartScriptPackageManifest } from './legacy-start-script-migration.interfaces.js'
...
const readPackageJson = (host: Tree, path: string): LegacyStartScriptPackageManifest => {
  ...
  return JSON.parse(stripJsonComments(buffer.toString('utf-8'))) as LegacyStartScriptPackageManifest
}

const isServiceWorkspaceManifest = (manifest: LegacyStartScriptPackageManifest): boolean =>
  manifest.scripts?.build === SERVICE_BUILD_SCRIPT && manifest.scripts.dev === SERVICE_DEV_SCRIPT

const isRendererWorkspaceManifest = (manifest: LegacyStartScriptPackageManifest): boolean =>
  manifest.scripts?.build === RENDERER_BUILD_SCRIPT && manifest.scripts.dev === RENDERER_DEV_SCRIPT

const shouldUpdateServiceStartScript = (manifest: LegacyStartScriptPackageManifest): boolean =>
  isServiceWorkspaceManifest(manifest) && manifest.scripts?.start === LEGACY_SERVICE_START_SCRIPT

const shouldUpdateRendererStartScript = (manifest: LegacyStartScriptPackageManifest): boolean =>
  isRendererWorkspaceManifest(manifest) && manifest.scripts?.start === LEGACY_SERVICE_START_SCRIPT

const updateServiceStartScript = (
  manifest: LegacyStartScriptPackageManifest
): LegacyStartScriptPackageManifest => ({
  ...manifest,
  scripts: {
    ...manifest.scripts,
...
const updateRendererStartScript = (
  manifest: LegacyStartScriptPackageManifest
): LegacyStartScriptPackageManifest => ({
  ...manifest,
  scripts: {
...
export const migrateLegacyStartScripts = (): Rule =>
