...
import { buildProjectCollection }     from './artifact.js'
...
const getChangedPaths = (
  result: Awaited<ReturnType<typeof scaffoldProjectWithAngular>>
): Array<string> => result.changes.map(({ path }) => path).sort()
...
const assertExactScaffoldFiles = async (scaffoldType: 'library' | 'project'): Promise<void> => {
  ...
  const result = await scaffoldProjectWithAngular(collectionPath, {
    scaffoldType,
    targetPath: target,
  })
  const scaffoldFiles =
...
test('should return a typed failure for an unavailable collection provider', async () => {
  ...
  const result = await scaffoldProjectWithAngular(join(fixtureRoot, 'missing/collection.json'), {
    scaffoldType: 'project',
    targetPath: target,
  })
...
test('should return a typed failure for a corrupt collection artifact', async () => {
  ...
  const result = await scaffoldProjectWithAngular(corruptCollectionPath, {
    scaffoldType: 'project',
    targetPath: target,
  })
