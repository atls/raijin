...
import { createGeneratedWorkflowPolicy } from './generated-workflow-policy.js'

test('should derive the generated Node line and current workflow values from Raijin metadata', () => {
  assert.deepEqual(
    createGeneratedWorkflowPolicy({ devDependencies: { '@types/node': '24.12.2' } }),
    {
...
test('should accept a ranged Node types version', () => {
  assert.equal(
    createGeneratedWorkflowPolicy({ dependencies: { '@types/node': '^30.1.0' } }).nodeVersion,
    '30'
  )
})
...
test('should reject a manifest without a semantic Node types version', () => {
  ...
  assert.throws(
    () => createGeneratedWorkflowPolicy({ devDependencies: { '@types/node': 'latest' } }),
    /does not declare a semver @types\/node version/
  )
})
