...
import type { WorkspaceManifest } from './start-scripts.interfaces.js'
...
const readPackageJson = (host: Tree, path: string): WorkspaceManifest => {
  ...
  return JSON.parse(stripJsonComments(buffer.toString('utf-8'))) as WorkspaceManifest
}

const isServiceWorkspaceManifest = (manifest: WorkspaceManifest): boolean =>
  manifest.scripts?.build === SERVICE_BUILD_SCRIPT && manifest.scripts.dev === SERVICE_DEV_SCRIPT

const isRendererWorkspaceManifest = (manifest: WorkspaceManifest): boolean =>
  manifest.scripts?.build === RENDERER_BUILD_SCRIPT && manifest.scripts.dev === RENDERER_DEV_SCRIPT

const shouldUpdateServiceStartScript = (manifest: WorkspaceManifest): boolean =>
  isServiceWorkspaceManifest(manifest) && manifest.scripts?.start === LEGACY_SERVICE_START_SCRIPT

const shouldUpdateRendererStartScript = (manifest: WorkspaceManifest): boolean =>
  isRendererWorkspaceManifest(manifest) && manifest.scripts?.start === LEGACY_SERVICE_START_SCRIPT

const updateServiceStartScript = (manifest: WorkspaceManifest): WorkspaceManifest => ({
  ...manifest,
  scripts: {
    ...manifest.scripts,
...
const updateRendererStartScript = (manifest: WorkspaceManifest): WorkspaceManifest => ({
  ...manifest,
  scripts: {
    ...manifest.scripts,
