...
const createSchematicArtifact = async (root: string): Promise<void> => {
...
test('should install schematic artifact next to runtime', async () => {
  const cwd = await mkdtemp(join(tmpdir(), 'raijin-schematic-install-'))
  const artifactDir = await mkdtemp(join(tmpdir(), 'raijin-schematic-artifact-'))

  await createSchematicArtifact(artifactDir)
  await installRaijinSchematicArtifact(cwd, artifactDir)
...
test('should replace stale schematic artifact', async () => {
  const cwd = await mkdtemp(join(tmpdir(), 'raijin-schematic-install-'))
  const artifactDir = await mkdtemp(join(tmpdir(), 'raijin-schematic-artifact-'))
