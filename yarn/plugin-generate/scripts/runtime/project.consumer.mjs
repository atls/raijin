...
/** @typedef {'library' | 'project'} ScaffoldType */
/**
 * @typedef {Error & {
 *   code?: number | string
...
const [runtimeArgument, packageManager, nodeLinker] = process.argv.slice(2)

if (!runtimeArgument || !packageManager || !['node-modules', 'pnp'].includes(nodeLinker)) {
  throw new Error('Usage: project.consumer.mjs <runtime-path> <package-manager> <pnp|node-modules>')
}
...
/** @type {Record<ScaffoldType, Array<string>>} */
const scaffoldFiles = {
 ...
 * @param {string} [cwd]
 */
const runYarn = async (args, cwd = fixtureRoot) =>
  execFileAsync('yarn', args, {
    cwd,
    encoding: 'utf8',
    env: environment,
  })
...
/**
 * @param {string} target
 * @param {ScaffoldType} scaffoldType
 */
const assertGeneratedScaffold = async (target, scaffoldType) => {
  assert.deepEqual(
 ...
 * @param {ScaffoldType} scaffoldType
 */
const assertGeneratedScaffoldCommand = async (target, scaffoldType) => {
...
try {
  await execFileAsync('yarn', ['workspace', '@atls/raijin', 'pack', '--out', packageArchivePath], {
    cwd: repoRoot,
    encoding: 'utf8',
    env: environment,
  })
  ...
  /** @type {Record<ScaffoldType | 'invalid', string>} */
  const targets = {
  ...
  assert.equal(actualVersionOutput.trim(), expectedVersionOutput.trim())
  ...
  await assert.rejects(
    runYarn(['generate', 'project', '--type', 'service'], targets.invalid),
    (error) => {
      const commandError = /** @type {CommandExecutionError} */ (error)
