...
describe('root project discovery', () => {
  ...
  test('finds an internal root config from a nested invocation', async (t) => {
    const cwd = await createProject({
    ...
    const diagnostics = checkProject(
      { kind: 'project', cwd: nestedCwd, projectCwd: cwd },
      undefined,
      ts
    )
  ...
  test('does not accept a config above the active project root', async (t) => {
    ...
    assert.equal(
      checkProject({ kind: 'project', cwd: projectCwd, projectCwd }, undefined, ts),
      undefined
    )
...
test('checks exact roots with project options but without project selection or references', async (t) => {
  const cwd = await createProject({
    'tsconfig.json': JSON.stringify({
      compilerOptions: { noUnusedLocals: true, strict: true },
      include: ['src/included.ts'],
      exclude: ['src/selected.ts'],
      references: [{ path: './packages/lib' }],
    }),
    'src/included.ts': 'export const included: MissingType = true\n',
    'src/selected.ts':
      "import { dependency } from './dependency.js'\n\nconst unused = true\nexport const selected: string = dependency\n",
    'src/dependency.ts': "export const dependency: number = 'dependency'\n",
  ...
  const before = await readTree(cwd)
  const selected = join(cwd, 'src/selected.ts')

  t.after(async () => rm(cwd, { recursive: true, force: true }))

  const diagnostics = checkProject(
    { kind: 'files', cwd, projectCwd: cwd, files: [selected] },
    undefined,
    ts
  )

  assertDiagnostics(diagnostics)
  assert.equal(hasDiagnostic(diagnostics, 6133, '/src/selected.ts'), true)
  assert.equal(hasDiagnostic(diagnostics, 2322, '/src/selected.ts'), true)
  assert.equal(hasDiagnostic(diagnostics, 2322, '/src/dependency.ts'), true)
  assert.equal(
    diagnostics.some(({ file }) => file?.fileName.endsWith('/src/included.ts')),
    false
  )
  assert.equal(
    diagnostics.some(({ file }) => file?.fileName.endsWith('/packages/lib/src/index.ts')),
    false
  )
  assert.deepEqual(await readTree(cwd), before)
})

test('checks transitive TypeScript-resolved references without prebuilt output or writes', async (t) => {
  const cwd = await createProject({
...
test('checks a shared TypeScript-resolved reference once', async (t) => {
  const cwd = await createProject({
    'tsconfig.json': JSON.stringify({
      files: [],
  ...
  const typescript = new Proxy(ts, {
    get: (runtime, property, receiver) => {
      ...
      return Reflect.get(runtime, property, receiver) as unknown
    },
  })

  t.after(async () => rm(cwd, { recursive: true, force: true }))

  const diagnostics = checkProject({ kind: 'project', cwd, projectCwd: cwd }, undefined, typescript)
...
test('keeps project configuration failures as TypeScript diagnostics', async (t) => {
  const cwd = await createProject({
