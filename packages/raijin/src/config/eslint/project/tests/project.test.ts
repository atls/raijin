...
test('should use a project config without injecting Raijin defaults', async () => {
  ...
  const configSource = `export default [{ files: ['**/*.ts'], plugins: { 'react-hooks': { rules: {} } }, rules: { 'no-console': 'off' } }]\n`
  ...
  const options = await resolveEslintProject({ cwd, eslint: ESLint, rootCwd: cwd })
  const eslint = new ESLint(options)
  const [result] = await eslint.lintFiles([filePath])

  assert.equal(options.baseConfig, undefined)
  assert.equal(options.overrideConfigFile, undefined)
  assert.equal(
...
test('should use defaults when project config is absent', async () => {
  ...
  assert.equal(options.overrideConfigFile, true)
  assert.equal(
    result.messages.some(({ ruleId }) => ruleId === 'no-console'),
    true
  )
...
test('should apply explicitly imported Raijin defaults before project rules', async (t) => {
  ...
  const cwd = await mkdtemp(join(repoRoot, '.raijin-eslint-opt-in-'))
  const filePath = join(cwd, 'index.ts')
  ...
  await writeFile(
    ...
    "import { eslintconfig } from '@atls/raijin/eslint'\nexport default [...eslintconfig, { files: ['**/*.ts'], rules: { 'no-console': 'off' } }]\n"
  )
  await writeFile(filePath, 'console.log("value")\n')
